...
void Player::tick() {
    ...
    if(input::isKeyDown(SDL_SCANCODE_W) && onGround){ velocityY = jumpSpeed; onGround = false;}
    if(input::isKeyDown(SDL_SCANCODE_A)){velocityX -= moveSpeed; lastDirection = Direction::LEFT;}
    if(input::isKeyDown(SDL_SCANCODE_D)){velocityX += moveSpeed; lastDirection = Direction::RIGHT;}
