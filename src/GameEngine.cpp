...
void GameEngine::handlePlayerLedgeCollisions() {
    for (auto& sprite : sprites) {
        if (auto player = std::dynamic_pointer_cast<Player>(sprite)) {
            player->setOnGround(false);
            SDL_FRect p = player->getRect();

            for (auto& other : sprites) { // Loopar genom alla sprites för att hitta ledges
...
// Hantera kollision mellan Player och Enemy
void GameEngine::handlePlayerEnemyCollisions(bool& running){ { 
        for (auto& sprite : sprites) {
            if (auto player = std::dynamic_pointer_cast<Player>(sprite)) {
                SDL_FRect p = player->getRect();
                for (auto& other : sprites) { // Loopar genom alla sprites för att hitta enemies
...
void GameEngine::handlePlayerProjectileEnemyCollisions(){
    for (auto& sprite : sprites) { 
...
void GameEngine::updateCameraAndGround() {
    ...
    for (auto& sprite : sprites) {
        if (auto player = std::dynamic_pointer_cast<Player>(sprite)) {
            // Kameran följer spelaren, men med en fjärdedels skärm som offset
            cameraX = std::max(0.0f, player->getX() - constants::gScreenWidth / 4.0f);
            break;
        }
    }

    // Flytta bottenledgen så den alltid täcker skärmen
    for (auto& sprite : sprites) {
...
void GameEngine::regeneratePlayerHealth(float amount) {
    for (auto& sprite : sprites) {
    if (auto player = std::dynamic_pointer_cast<Player>(sprite)) {
...
void GameEngine::render() { // Ritar bakgrund och sprites
    SDL_SetRenderDrawColor(renderer, 0, 0, 0, 255);
    SDL_RenderClear(renderer);
    // Ritar bakgrunden med parallax effekt
    float bgWidth = constants::gScreenWidth;
    float bgOffsetX = fmod(cameraX, bgWidth);
    for (int i = -1; i <= 2; i++) { // Ritar extra bakgrundsbilder för att täcka hela skärmen
